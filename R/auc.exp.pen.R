...
auc.exp.pen <- function(x, y, penalty=c("lasso","ridge","scad"), lambda=lambda, learn.rate=learn.rate,
                        eps=eps, max.iter=max.iter, a=a){
  # scale
  x <- scale(x, scale=F)
  y <- y
  p <- ncol(x)
  ...
  if(penalty == "lasso"){
    ...
    for(iter in 1:max.iter){
      ...
      grad.v <- apply(exp(-c(t(m.ij))) * derv.m, 2, mean)
    ...
    for(iter in 1:max.iter){
      ...
      x_m = x[i_2,]
      derv.m <- -(x_p[rep(1:nrow(x_p), times=rep(length(i_2),nrow(x_p))),] - x_m[rep(1:length(i_2), length(i_1)),])
      grad.v <- apply(exp(-c(t(m.ij))) * derv.m, 2, mean) + lambda*beta.old
    ...
    t.s <- learn.rate
    beta.c = rep(0,p)
    beta.init <- penAUC(x, y, loss="exponential", penalty="ridge", lambda, learn.rate, eps, max.iter)
    ...
    for(i in 1:max.iter){
      ...
      for(iter in 1:max.iter){
        ...
        derv.m <- -(x_p[rep(1:nrow(x_p), times=rep(length(i_2),nrow(x_p))),] - x_m[rep(1:length(i_2), length(i_1)),])
        grad.v <- apply(exp(-c(t(m.ij))) * derv.m, 2, mean) + pen.f(beta.c, a, lambda)*sign(beta.c)
  ...
  val <- structure(list(beta=round(beta.new, 4), lambda=lambda, penalty=penalty), class="auc.exp.pen")
